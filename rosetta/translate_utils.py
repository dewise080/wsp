...
def translate(text, from_language, to_language):
    LIBRETRANSLATE_URL = getattr(
        settings, "LIBRETRANSLATE_URL", "https://translate.lotfinity.tech"
    )
    ...
    if LIBRETRANSLATE_URL:
        return translate_by_libretranslate(
            text, from_language, to_language, LIBRETRANSLATE_URL
        )
    if DEEPL_AUTH_KEY:
...
def translate_by_libretranslate(text, from_language, to_language, base_url):
    """
    Translate text using a LibreTranslate-compatible API.
    :param text: Source text to translate.
    :param from_language: Source language code (e.g., en, en-us).
    :param to_language: Target language code (e.g., tr, tr-tr).
    :param base_url: Base URL of the LibreTranslate service.
    """
    # LibreTranslate expects short language codes like "en" or "tr".
    source = from_language.split("-", 1)[0]
    target = to_language.split("-", 1)[0]
    endpoint = base_url.rstrip("/") + "/translate"
    timeout = getattr(settings, "LIBRETRANSLATE_TIMEOUT", 30)
    retries = getattr(settings, "LIBRETRANSLATE_RETRIES", 2)
    ...
    for attempt in range(retries + 1):
        try:
            ...
            last_error = err
            if attempt == retries:
                raise TranslationException(f"LibreTranslate connection error: {err}")
            # brief backoff before retrying
            continue

    if response.status_code != 200:
        raise TranslationException(
            f"LibreTranslate response is {response.status_code}: {response.text}"
        )
    ...
    try:
        ...
        if not translated:
            raise TranslationException("LibreTranslate returned an unexpected response.")
        return translated
    except (ValueError, KeyError, TypeError) as err:
        raise TranslationException(f"LibreTranslate parse error: {err}")
